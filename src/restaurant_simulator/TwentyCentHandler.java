...
/**
 * Classe venti centesimi handler
 * 
 * @author Danilo Dolce e Simone Contini
 * @version 1.0
 */
public class TwentyCentHandler implements IBanknoteHandler {
    ...
    public void handlerRequest(Banknote banknote) {
        if (banknote.getValue()>=0.2) {
            // effettuo l'arrotondamento per ricavarmi il corretto valore della banconota
            double valore = (double) Math.round((banknote.getValue()-0.20)*100)/100;
            banknote.setValue(valore);
            banknote.addBanknote("20 centesimi");
