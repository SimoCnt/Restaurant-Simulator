...
/**
 * Classe cinquanta centesimi handler
 * 
 * @author Danilo Dolce e Simone Contini
 * @version 1.0
 */
public class FiftyCentHandler implements IBanknoteHandler {
    ...
    public void handlerRequest(Banknote banknote) {
        if (banknote.getValue()>=0.5) {
            // effettuo l'arrotondamento per ricavarmi il corretto valore della banconota
            double valore = (double) Math.round((banknote.getValue()-0.50)*100)/100;
            banknote.setValue(valore);
            banknote.addBanknote("50 centesimi");
